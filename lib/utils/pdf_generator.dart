...
class PDFGenerator {
  ...
  Future<File> generateSummaryPDF(model.User currentUser,
      List<FoodOrder> buyerOrders, List<FoodOrder> sellerOrders) async {
    final pdf = pw.Document();

    final buyerOrderDetails =
        await _fetchOrderDetails(buyerOrders, isBuyer: true);
    final sellerOrderDetails =
        await _fetchOrderDetails(sellerOrders, isBuyer: false);

    final totalBuyerAmount =
        buyerOrders.fold<int>(0, (sum, order) => sum + order.totalPrice);
    final totalSellerAmount =
        sellerOrders.fold<int>(0, (sum, order) => sum + order.totalPrice);
    ...
    pdf.addPage(pw.Page(
      build: (pw.Context context) {
        return pw.Column(
          ...
          children: [
            pw.Text('Rangkuman Pesanan di Mealty',
                style:
            ...
            pw.Align(
              ...
              child: pw.Text(
                  'Total Pembelian: ${formatPrice(totalBuyerAmount)}',
            ...
            pw.Align(
              alignment: pw.Alignment.centerRight,
              child: pw.Text(
                  'Total Penjualan: ${formatPrice(totalSellerAmount)}',
                  style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
            ),
          ],
        );
    ...
    final fileName =
        'summary-${DateFormat('yyyyMMddHHmmss').format(DateTime.now())}.pdf';
    final file = File('$downloadsPath/$fileName');
    await file.writeAsBytes(await pdf.save());
    return file;
  }

  Future<List<pw.Widget>> _fetchOrderDetails(List<FoodOrder> orders,
      {required bool isBuyer}) async {
    ...
    for (var order in orders) {
      ...
      for (var item in order.foodItems) {
        ...
        final foodData = foodDoc?.data()
            as Map<String, dynamic>?; // Ensure proper type checking
